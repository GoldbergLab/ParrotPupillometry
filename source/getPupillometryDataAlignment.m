...
if isempty(click_struct)
    ...
    click_struct = makeAudioSyncStruct( ...
        ...
        'FileRegex', options.AudioFileRegex, ...
        'PulseShape', options.AudioPulseShape, ...
        'PlotOnsets', false ...
        );
    disp('...done')
    if options.SaveStructs
...
% Fail clearly if no clicks were detected, rather than crashing later in
%   fuzzyMatchEvents when it tries to match against an empty event list.
if isempty([click_struct.onsets_cumulative])
    error('getPupillometryDataAlignment:noClicksDetected', ...
        ['No sync clicks were detected in any audio file (click channel %d).\n' ...
         'Check the ClickChannel, the detection threshold, and AudioPulseShape ' ...
         '(''level'' for a directly-recorded sustained LED pulse, ''pair'' for the ' ...
         'legacy relay/microphone two-click signal).'], options.ClickChannel);
end
...
if isempty(naneye_flash_struct) && options.IncludeNaneye
    ...
    for s = 1:num_naneye_streams
        ...
        flash_struct = makeVideoSyncStruct( ...
            root, ...
            naneye_roi(s, :), ...
            options.NaneyeThreshold, ...
            0.08, ...
            'FrameRate', 48, ...
            'FileRegex', options.NaneyeFileRegex, ...
            'MedianWindow', options.NaneyeMedianWindow, ...
            'RangeBasedThreshold', options.NaneyeRangeBasedThreshold, ...
            'PlotOnsets', false, ...
            'NumIgnoredFlashes', naneye_num_ignored(s), ...
            "FileLimit", options.FileLimit, ...
            "BadSyncFileIdx", naneye_bad_sync{s} ...
            );
        if isempty([flash_struct.onsets_cumulative])
            error('getPupillometryDataAlignment:noNaneyeFlashes', ...
                ['No sync flashes were detected for naneye %s (ROI [%s]).\n' ...
                 'Check that the NaneyeROI for this eye covers its sync LED and ' ...
                 'that the detection threshold is appropriate.'], ...
                naneye_labels{s}, num2str(naneye_roi(s, :)));
        end
...
if isempty(webcam_flash_struct) && options.IncludeWebcam
    ...
    webcam_flash_struct = makeVideoSyncStruct( ...
        root, ...
        options.WebcamROI, ...
        options.WebcamThreshold, ...
        0.08, ...
        'FrameRate', 45, ...
        'FileRegex', options.WebcamFileRegex, ...
        'MedianWindow', options.WebcamMedianWindow, ...
        'RangeBasedThreshold', options.WebcamRangeBasedThreshold, ...
        'PlotOnsets', false, ...
    ...
    disp('...done')
    if isempty([webcam_flash_struct.onsets_cumulative])
        error('getPupillometryDataAlignment:noWebcamFlashes', ...
            ['No sync flashes were detected for the webcam (ROI [%s]).\n' ...
             'Check that WebcamROI covers the sync LED and that the detection ' ...
             'threshold is appropriate.'], num2str(options.WebcamROI));
    end
